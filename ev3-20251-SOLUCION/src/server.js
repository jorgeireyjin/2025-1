/**
 * Todas las rutas reciben y envían datos en formato JSON
 */

const express = require('express');
const app = express();
const port = 3000;

// Middleware para parsear JSON en las solicitudes
app.use(express.json());

/*
Definimos los pesos de las evaluaciones
*/
const pesos = new Map();
pesos.set('EP', 0.3);
pesos.set('EF', 0.4);
pesos.set('TA', 0.3);
pesos.set('P1', 0.1);
pesos.set('P2', 0.1);
pesos.set('P3', 0.1);
pesos.set('P4', 0.2);
pesos.set('PR', 0.3);

/* 
Aqui almacenamos los datos
*/
const datos = [];

/*
Aqui guardamos los promedios calculados
*/
var promedios = [];

/*
Pregunta 1:
La api debe recibir un archivo JSON con las notas de un alumno y almacenarlas
de tal forma que luego se puedan procesar
*/
app.post('/api/pregunta1', (req, res) => {
  const data = req.body;
  console.log(data);
  datos.push(data);
  res.json({ 'msg': `ALumno agregado  ... van ${datos.length}` });
});

/*
Pregunta 2:
La api debe recibir un archivo JSON con el codigo del alumno
Debe devolver todas sus notas
*/
app.get('/api/pregunta2', (req, res) => {
  const data = req.body;
  const codigo = data.codigo;

  const rpta =  datos.filter(alumno => alumno.codigo == codigo);

  res.json(rpta);
});

/*
Pregunta 3:
La api no recibe parametros.
Calcula loss  y devuelve los codigos de los alumnos y su promedio final
*/
app.get('/api/pregunta3', (req, res) => {

  promedios = datos.map((alumno) => {
    let ta = alumno.P1 * pesos.get('P1') + alumno.P2 * pesos.get('P2') + alumno.P3 * pesos.get('P3') + alumno.P4 * pesos.get('P4') + alumno.PR * pesos.get('PR')
    let prom = alumno.EP * pesos.get('EP') + alumno.EF * pesos.get('EF') + ta * pesos.get('TA')
    const obj = { codigo: alumno.codigo, promedio: prom }
    return obj
  });
  res.json(promedios);

});

/*
Pregunta 4:
La api no recibe parametros.
Devuelve al alumno o alumnos que superan dicho promedio.
Debe retornar el código y promedio final de cada alumno en dicha situación
*/
app.get('/api/pregunta4', (req, res) => {

  // Primero hay que obtener el menor promedio
  console.log(`PROMEDIOS : ${promedios}`)
  let menor = 9999;

  // Encontrar el menor promedio
  promedios.forEach(obj => {
    console.log(`OBJ ${obj.codigo} - ${obj.promedio}`);
    if (obj.promedio < menor) {
      menor = obj.promedio;
    }
  });

  console.log(`PROMEDIO MENOR : ${menor}`)

  // Filtrar los objetos que tienen el promedio menor o igual al menor
  let rpta = promedios.filter(obj => obj.promedio > menor);;
  res.json(rpta);

});


// Iniciar servidor
app.listen(port, () => {
  console.log(`Servidor escuchando en http://localhost:${port}`);
});

